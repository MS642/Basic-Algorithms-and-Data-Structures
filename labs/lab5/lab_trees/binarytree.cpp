...
template <typename T>
void BinaryTree<T>::printPath(const Node* subRoot, vector<T> arr) const
{
    ...
    printPath(subRoot->left, v);
    printPath(subRoot->right, v);

}


template <typename T>
void BinaryTree<T>::print_arr(vector<T> arr) const 
{
    if (arr.size() == 0) {
        return;
    }
...
{
    int counter = -1;
    int result = sumDistances(root, counter);
    return result;
}
...
template <typename T>
bool BinaryTree<T>::isOrdered(const Node* node) const
{
    ...
    if (isOrdered(node->left)) {
        if (isOrdered(node->right)) {
            return true;
        }
        else {
            return false;
